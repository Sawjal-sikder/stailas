...
✨ Style Suggestion: Consider adding a statement necklace or colorful scarf for a bolder touch. A pastel blazer (lavender, mint, or peach) would really make this outfit pop!`,
  ...
  return (
    <>
      <Navbar />
      <div className='max-w-7xl mx-auto p-5 mt-10 rounded-md shadow-md font-inter bg-white'>
          ...
          {chat.map((message) => (
            <div
              ...
              className={`flex items-start ${
                message.user_id === 1 ? 'justify-start' : 'justify-end'
              }`}
            >
              {/* Left user image */}
              {message.user_id === 1 && (
              ...
              {/* Message bubble */}
              <div
                className={`px-4 py-2 rounded-lg max-w-3xl text-sm ${
                  message.user_id === 1
